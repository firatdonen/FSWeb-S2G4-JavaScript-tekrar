...
function CemberinAlani(yaricap) {
  return pi * Math.pow(yaricap, 2)//yaricap*yaricap
}
const sonuc=CemberinAlani(15);
console.log("Çemberin Alanı:",sonuc)
...
enbuyuk = [0]; //index deki ilk elamanı alıyoruz 
enkucuk = [0]; //index deki ilk elamanı alıyoruz 
...
besyuzdenkucuksayilar = [];
sayilar.filter((deger) => {
  if (deger < 500) {
    besyuzdenkucuksayilar.push(deger)
  }
}
);
...
siralisayilar=besyuzdenkucuksayilar;//sıralı sayıları sıralisayilar adlı diziye aktarır.


// 3f çözümü

tekraredensayilar=sayilar.filter((sayi,index)=>sayilar.indexOf(sayi) !== index); 

const string={};
tekraredensayilar.forEach((sayi)=>{
  if(!string[i])
    {string[i]=1 //daha önce tanımlanmamaış, ilk kez görüyor  ve başka görmüyorsa
  }
  else{
    string[i]++
  } 
